...
class MLOptimizer(nn.Module):
    ...
    @staticmethod
    def get_density_descriptors(nn_inputs, rho_a_inp, rho_b_inp, grad_a_inp, grad_b_inp, grad_inp, tau_a_inp, tau_b_inp):
    ...
    def forward(self, nn_inputs, rho_a_inp, rho_b_inp, grad_a_inp, grad_b_inp, grad_inp, tau_a_inp, tau_b_inp):
        '''
        Returns:
            spin-symmetrized enhancement factor for LDA exhange energy
        '''

        x = self.get_density_descriptors(nn_inputs, rho_a_inp, rho_b_inp, grad_a_inp, grad_b_inp, grad_inp, tau_a_inp, tau_b_inp)
...
class pcPBEMLOptimizer(nn.Module):
    ...
    def forward(self, nn_inputs, rho_a_inp, rho_b_inp, grad_a_inp, grad_b_inp, grad_inp, tau_a_inp, tau_b_inp):
        ...
        x_exchange_desc = self.get_density_descriptors(nn_inputs, 
                                                    2*rho_a_inp, 2*rho_b_inp,
                                                    4*grad_a_inp, 4*grad_b_inp, 4*grad_inp,
                                                    2*tau_a_inp, 2*tau_b_inp)
        
        x_correlation_desc = self.get_density_descriptors(nn_inputs,
                                                        rho_a_inp, rho_b_inp,
                                                        grad_a_inp, grad_b_inp, grad_inp,
                                                        tau_a_inp, tau_b_inp)

        x_correlation_desc_swapped = x_correlation_desc[:, [1, 0, 4, 3, 2, 6, 5]]
        params_c_real = (self.hidden_layers_c(x_correlation_desc) + self.hidden_layers_c(x_correlation_desc_swapped)) / 2
        beta_real, gamma_real = params_c_real[:, 0].view(-1, 1), params_c_real[:, 1].view(-1, 1)
        ...
        params_c_sigma_zero = (self.hidden_layers_c(x_corr_sigma_zero) + self.hidden_layers_c(x_corr_sigma_zero_swapped)) / 2
        ...
        params_c_rho_inf = (self.hidden_layers_c(x_corr_rho_inf) + self.hidden_layers_c(x_corr_rho_inf_swapped)) / 2
        ...
        mu_up_real, kappa_up_real = params_x_up_real[:, 0].view(-1, 1), params_x_up_real[:, 1].view(-1, 1)
        mu_down_real, kappa_down_real = params_x_down_real[:, 0].view(-1, 1), params_x_down_real[:, 1].view(-1, 1)
        ...
        constants_batch = true_constants_PBE.repeat(x_exchange_desc.shape[0], 1).to(x_exchange_desc.device)
        fill_tensor = torch.ones([x_exchange_desc.shape[0], 20], device=x_exchange_desc.device)
...
class pcPBEdoublestar(pcPBEMLOptimizer):
    ...
    def __init__(self, num_layers, h_dim, nconstants_x=1, nconstants_c=1, dropout=0.2, DFT=None):
        super().__init__(num_layers, h_dim, nconstants_x=nconstants_x, nconstants_c=nconstants_c, dropout=dropout, DFT=DFT)
    ...
    def forward(self, nn_inputs, rho_a_inp, rho_b_inp, grad_a_inp, grad_b_inp, grad_inp, tau_a_inp, tau_b_inp):
        ...
        for i in range(num_constraints):
            ...
            for j in range(num_constraints):
                if i != j:
                    numerator = numerator * self._compute_l(x_correlation_desc, constraint_points[j])
                    denominator = denominator * self._compute_l(constraint_points[i], constraint_points[j])
...
def test_model_constraints(model, model_name):
    ...
    BATCH_SIZE = 8
    model.eval()
    nn_inputs_placeholder = torch.zeros(BATCH_SIZE, 7, device=next(model.parameters()).device)
    all_passed = True
    ...
    if isinstance(model, pcPBEdoublestar):
        ...
        Fx_up, Fx_down, _ = model(nn_inputs_placeholder, rho_a, rho_b, grad_zero, grad_zero, grad_zero, tau_a_test, tau_b_test)
        fx_up_passed = torch.allclose(Fx_up, torch.ones_like(Fx_up))
        fx_down_passed = torch.allclose(Fx_down, torch.ones_like(Fx_down))
        print(f"Fx (spin-up) -> 1.0? {'PASS' if fx_up_passed else 'FAIL'} (Mean: {Fx_up.mean().item():.6f})")
        print(f"Fx (spin-down) -> 1.0? {'PASS' if fx_down_passed else 'FAIL'} (Mean: {Fx_down.mean().item():.6f})")
        if not (fx_up_passed and fx_down_passed): all_passed = False
        ...
        _, _, Fc_ueg = model(nn_inputs_placeholder, rho_a, rho_b, grad_zero, grad_zero, grad_zero, tau_tf_rho_a, tau_tf_rho_b)
        fc_ueg_passed = torch.allclose(Fc_ueg, torch.ones_like(Fc_ueg))
        print(f"Fc -> 1.0? {'PASS' if fc_ueg_passed else 'FAIL'} (Mean: {Fc_ueg.mean().item():.6f})")
        if not fc_ueg_passed: all_passed = False
        ...
        _, _, Fc_hd = model(nn_inputs_placeholder, rho_large, rho_large, rand_grads, rand_grads, rand_grads*2, rand_taus, rand_taus)
        fc_hd_passed = torch.allclose(Fc_hd, torch.ones_like(Fc_hd), atol=1e-5)
        print(f"Fc -> 1.0? {'PASS' if fc_hd_passed else 'FAIL'} (Mean: {Fc_hd.mean().item():.6f})")
        if not fc_hd_passed: all_passed = False

    elif isinstance(model, pcPBEMLOptimizer):
        true_factors = true_constants_PBE.repeat(BATCH_SIZE, 1).to(nn_inputs_placeholder.device)
        ...
        tau_a_test = tau_tf_2rho_a / 2
        tau_b_test = tau_tf_2rho_b / 2
        output_mu = model(nn_inputs_placeholder, rho_a, rho_b, grad_zero, grad_zero, grad_zero, tau_a_test, tau_b_test) / true_factors
        ...
        print(f"Mu (spin-up) -> 1.0? {'PASS' if mu_up_passed else 'FAIL'} (Mean: {mu_up.mean().item():.6f})")
        print(f"Mu (spin-down) -> 1.0? {'PASS' if mu_down_passed else 'FAIL'} (Mean: {mu_down.mean().item():.6f})")
        if not (mu_up_passed and mu_down_passed): all_passed = False

        print("\n[TEST 2/4] Checking `beta` constraint (Correlation UEG Limit)...")
        tau_tf_rho_a = 3/10 * (3*np.pi**2)**(2/3) * rho_a**(5/3)
        tau_tf_rho_b = 3/10 * (3*np.pi**2)**(2/3) * rho_b**(5/3)
        output_beta = model(nn_inputs_placeholder, rho_a, rho_b, grad_zero, grad_zero, grad_zero, tau_tf_rho_a, tau_tf_rho_b) / true_factors
        beta = output_beta[:, 0]
        beta_passed = torch.allclose(beta, torch.ones_like(beta))
        print(f"Beta -> 1.0? {'PASS' if beta_passed else 'FAIL'} (Mean: {beta.mean().item():.6f})")
        if not beta_passed: all_passed = False
        ...
        output_gamma = model(nn_inputs_placeholder, rho_large, rho_large, rand_grads, rand_grads, rand_grads, rand_taus, rand_taus) / true_factors
        gamma = output_gamma[:, 1]
        gamma_passed = torch.allclose(gamma, torch.ones_like(gamma))
        print(f"Gamma -> 1.0? {'PASS' if gamma_passed else 'FAIL'} (Mean: {gamma.mean().item():.6f})")
        if not gamma_passed: all_passed = False
        ...
        out1 = model(nn_inputs_placeholder, rho1, rho2, grad1, grad2, grad1+grad2, tau1, tau2)
        out2 = model(nn_inputs_placeholder, rho2, rho1, grad2, grad1, grad1+grad2, tau2, tau1)
        corr_symm = torch.allclose(out1[:, 0], out2[:, 0]) and torch.allclose(out1[:, 1], out2[:, 1])
        exch_swap = torch.allclose(out1[:, 22], out2[:, 24]) and torch.allclose(out1[:, 24], out2[:, 22])
        print(f"  > Correlation parameters are symmetric? {'PASS' if corr_symm else 'FAIL'}")
        print(f"  > Exchange parameters correctly swapped? {'PASS' if exch_swap else 'FAIL'}")
        if not (corr_symm and exch_swap): all_passed = False
