...
def main():
    ...
    if not mf.converged:
        ...
        if latest_delta_e is not None:
            with open("./non_converged_systems_density.log", "a") as file:
                log_line = (f"{functional}-{molecule_name if molecule_name else atom_name}: Not converged. "
    ...
    if molecule_name:
        ...
        INPUT_DIR = os.path.join(FUNCTIONAL_DIR, f"{atom_name}_+{charge}_{functional.replace("_pyscf", "")}.wfn")
