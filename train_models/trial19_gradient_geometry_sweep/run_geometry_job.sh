...
: "${GEOMETRY_MODE:?GEOMETRY_MODE must be set by the SLURM file}"
: "${GEOMETRY_TAG:?GEOMETRY_TAG must be set by the SLURM file}"

SUBMIT_DIR="${SLURM_SUBMIT_DIR:-$PWD}"
if [[ -f "$SUBMIT_DIR/replay_trial_19_gradient_geometry.py" ]]; then
    WORK_DIR="$SUBMIT_DIR"
elif [[ -f "$SUBMIT_DIR/train_models/replay_trial_19_gradient_geometry.py" ]]; then
    WORK_DIR="$SUBMIT_DIR/train_models"
else
    echo "Could not locate replay_trial_19_gradient_geometry.py from: $SUBMIT_DIR" >&2
    exit 1
fi
...
CUBLAS_WORKSPACE_CONFIG=:16:8 torchrun \
    ...
    replay_trial_19_gradient_geometry.py \
    --aggregation "$GEOMETRY_MODE" \
    --output-dir "optuna_joint_runs/replay_trial_19_geometry_${GEOMETRY_TAG}_500_gc_svelu_mirror" \
