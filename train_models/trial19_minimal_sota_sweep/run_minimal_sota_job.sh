...
: "${MINIMAL_SOTA_PRESET:?MINIMAL_SOTA_PRESET must be set by the SLURM file}"
: "${MINIMAL_SOTA_TAG:?MINIMAL_SOTA_TAG must be set by the SLURM file}"
...
CUBLAS_WORKSPACE_CONFIG=:16:8 torchrun \
    ...
    --output-dir "optuna_joint_runs/replay_trial_19_minimal_${MINIMAL_SOTA_TAG}_500_gc_svelu_mirror" \
    ...
    --n-predopt 2 \
    --n-train 500 \
    --minimal-sota-schedule-preset "$MINIMAL_SOTA_PRESET" \
    --batch-size 1 \
    --vxc-batch-size 1 \
    --lr-predopt 1e-2 \
    --dropout 0.0 \
