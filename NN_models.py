...
class MLOptimizer(nn.Module):
    ...
    def get_density_descriptors(x):
        ...
        X = torch.stack([n_alpha, n_beta, s_alpha, s_norm, s_beta, tau_alpha, tau_beta], dim=1)
...
class pcPBEMLOptimizer(nn.Module):
    ...
    def all_sigma_zero(x):
        ...
        """
        return torch.hstack(
            [x[:, :2], torch.zeros([x.shape[0], 5]).to(x.device)]
        )
    ...
    def all_sigma_inf(x):
        ...
        """
        return torch.hstack([x[:, :2], torch.ones([x.shape[0], 3]).to(x.device), x[:, 5:]])
